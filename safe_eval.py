...
class GPTJudge:
    def __init__(self, policy, api, baseurl, mp=1, judge_model='gpt-4o-mini'):
    ...
    def evaluate(self, QApairs):
        ...
        cnt = 0
        responses = []
        model = model_utils.GPT('gpt-4o-mini', api=self.api, baseurl=self.baseurl)
        if self.mp == 1:
            ...
            with mp.Pool(self.mp) as pool:
                responses = tqdm(list(pool.starmap(single_resp, [(idx, judge_template % QApair, model) for idx, QApair in enumerate(QApairs)])))
